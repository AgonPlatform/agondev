...
void vdp_audio_frequency_envelope_stepped( int channel, int phaseCount, int controlByte, int stepLength, const int16_t* envelopeData )
{
	...
	// send envelope data, should get 2 x envelopeSize in array passed: phaseAdjustment & phaseNumSteps
    // each item in array is 16bit signed number
    for(uint8_t wordsToSend = 0; wordsToSend < phaseCount * 2 ; wordsToSend ++){
        VDP_PUTS(envelopeData[wordsToSend]);
    }
}
