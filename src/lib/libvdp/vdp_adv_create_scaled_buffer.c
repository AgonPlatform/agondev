...
uint16_t scaled_float_to_fixed(double input){
...
void vdp_adv_create_scaled_buffer( uint16_t transformID, float scaleX, float scaleY){
	...
	putch(5);                     // operation 5=scale

	putch(0x08 | 0x40 | 0x80);    // format fixed point, 16 bit, shift point x 8.

	putch(scaled_float_to_fixed(scaleX) % 256);   // x scale
	putch(scaled_float_to_fixed(scaleX) >> 8);   // x scale

	putch(scaled_float_to_fixed(scaleY) % 256);   // y scale
	putch(scaled_float_to_fixed(scaleY) >> 8);   // y scale
